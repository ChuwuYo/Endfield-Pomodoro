...
- **双模式播放**:
  - **本地模式**: 支持拖拽导入 MP3/WAV/OGG，具备可视化频谱与播放列表管理
  - **在线模式**: 集成 MetingJS，支持网易云/QQ音乐/酷狗等平台的歌单/专辑/单曲解析
- **终端交互**: 极简终端风格界面，支持歌词显示 (LRC)、封面展示及后台播放

### 📋 任务与配置 (Mission & Config)
- **任务协议**: 模拟终端存储限制（Max 6），强制聚焦当前目标
- **系统个性化**:
  - **8种战术主题**: 从工业黑橙到赛博霓虹，每种主题包含独特的动态背景粒子
  - **国际化**: 中/英双语一键切换
